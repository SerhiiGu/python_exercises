...
"Get out!" she pulsed.
    ...
    write_to_file(text, file1)
    timeend = datetime.now()
    print(f'Write in single thread: {timeend - timestart}')

    timestart = datetime.now()
    t1 = Thread(target=write_to_file, args=(text, file1))
    t2 = Thread(target=write_to_file, args=(text, file2))
    t1.start()
    t2.start()
    t1.join()
    t2.join()
    timeend = datetime.now()
    print(f'Write in double thread: {timeend - timestart}')

    timestart = datetime.now()
    p1 = Process(target=write_to_file, args=(text, file1))
    p2 = Process(target=write_to_file, args=(text, file2))
    p1.start()
    p2.start()
    p1.join()
    p2.join()
    timeend = datetime.now()
    print(f'Write in double process: {timeend - timestart}')


# Results #
# Write in single thread: 0:00:00.886276
# Write in double thread: 0:00:01.460588
# Write in double process: 0:00:01.032999

# Write in single thread: 0:00:00.817000
# Write in double thread: 0:00:01.425418
# Write in double process: 0:00:01.088002
